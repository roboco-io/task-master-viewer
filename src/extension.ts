...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('task-master-viewer.openTaskFile', async (taskId: string, fileName: string) => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('task-master-viewer.sortTasks', async () => {
            const currentSort = provider.getSortOrder();
            const sortOptions = [
                { label: '📋 ID (Task Number)', value: 'id' as const, description: currentSort === 'id' ? 'Currently selected' : '' },
                { label: '🔥 Priority', value: 'priority' as const, description: currentSort === 'priority' ? 'Currently selected' : '' },
                { label: '📊 Status', value: 'status' as const, description: currentSort === 'status' ? 'Currently selected' : '' },
                { label: '🔤 Title', value: 'title' as const, description: currentSort === 'title' ? 'Currently selected' : '' }
            ];
